...
class raster_data:
    ...
    def match_extent(self,other_raster):
        new_profile=other_raster.profile
        new_profile['transform']=Affine(self.res,new_profile['transform'][1],new_profile['transform'][2],new_profile['transform'][3],-self.res,new_profile['transform'][5])    
        resolution_reduce=int(self.res/other_raster.res)
        new_profile['width']=new_profile['width']//resolution_reduce
        new_profile['height']=new_profile['height']//resolution_reduce
        new_raster=raster_data(raster=np.ones((new_profile['height'],new_profile['width']))*(-1000),profile=new_profile)
        y_start=int((self.bounds[0]-other_raster.bounds[0])/self.res)         
        x_start=int((self.bounds[1]-other_raster.bounds[1])/self.res)
        new_raster.data[x_start:x_start+self.x,y_start:y_start+self.y]=self.data
        new_raster.bounds=other_raster.bounds
        return new_raster
    ...
    def visualize(self,cmap=matplotlib.cm.viridis,crop_x=[0,1],crop_y=[0,1],fig=None,ax=None,figsize=(10,10),title=None,boolean=False,discrete=None,alpha=1,cbar=True,draw_pixel_grid=False,grid_color="white",y=1.03,return_im=False,save_to=None,extent_match=False,title_fontsize=18,cbar_pad=0.05,fixed_cbar_ticks=None,custom_cbar_labels=None,aspect=None,force_vmin=None,force_vmax=None,cbar_tick_label_size=20,cbar_title=None,cbar_label_size=20):
        ...
        if extent_match:
            ...
            extent=None
        sc=ax.imshow(cropped.data,cmap=cmap,alpha=alpha,extent=extent,aspect=aspect,vmin=force_vmin,vmax=force_vmax)
        if cbar:
            ...
            if discrete is not None:
                ...
                bar = fig.colorbar(sc,cax=cax,orientation='horizontal',spacing='proportional',ticks=discrete/2+pos,boundaries=np.append(pos,pos[-1]+discrete),label=cbar_title)     
                bar.ax.set_xticklabels((pos).astype(type(discrete)))
            else:
                if fixed_cbar_ticks is None:
                    bar = fig.colorbar(sc,cax=cax,orientation='horizontal',label=cbar_title)
                else:
                    bar = fig.colorbar(sc,cax=cax,orientation='horizontal',ticks=fixed_cbar_ticks,label=cbar_title)
            if custom_cbar_labels is not None:
                bar.ax.set_xticklabels(custom_cbar_labels)
            bar.ax.tick_params(labelsize=cbar_tick_label_size)
            if cbar_title is not None:
                cax.set_xlabel(cbar_title,fontsize=cbar_label_size)
        if draw_pixel_grid: 
...
class skeletonizable_raster(raster_data):
    ...
    def skeletonization(self,levels=None,skeleton=None,verbose=True,verbose_concerned=False,tck=None,stage=1,visual_control=True,crop_x=[0,0.1],crop_y=[0,0.1],boolean=False,cmap1=plt.cm.viridis,cmap2=plt.cm.plasma,cmap3=plt.cm.gray):
        ...
        if nlevels==1:
            ...
            if boolean:
                skeleton.data=np.where(skeleton.data>0,1,0)
            else:
...
class DEM(raster_data):
    ...
    def __init__(self,wavelength_of_interest=15,RR_filter=None,VO_radius=None,VO_VE=1000,starting_visual=False,starting_visual_figsize=(15,6),precompute=False,precompute_VO=True,verbose=True,presmoothing=True,presmoothing_filter=3,paths_to_precomputed={},prepresmooth_visual=None,prepresmooth_crop=None,ignore_LoG=False,**kwargs):
        super().__init__(**kwargs)
        self.WOI=wavelength_of_interest
        self.ignore_LoG=ignore_LoG
        if RR_filter is None:
            RR_filter=self.WOI
        if prepresmooth_visual is not None:
            self.crop(prepresmooth_crop[0],prepresmooth_crop[1]).visualize(cmap=plt.cm.terrain,save_to=prepresmooth_visual,title='')
        if presmoothing:
    ...
    def dune_predictions_raw(self,method='slopes',AI_treshold=0.9,AI_model=None,AI_raw_output=None,AI_buffer=5,AI_low_limit=0.0010634725,AI_high_limit=0.99893653,AI_treshold_edge_buff=10,slope_treshold=0.1,slope_treshold_edge_extra=0.1,slope_treshold_edge_buff=10,LoG_treshold_edge_extra=0.04,LoG_treshold_edge_buff=10,LoG_treshold=0.05,name=None,visual_control=False,visual_control_figsize=(15,10),verbose=True,output_to_file=None,presmooth_input=0):
        ...
        if(method=='AI'):
            if verbose:
                ...
                AI_verb=None
            if presmooth_input==0:
                prediction_raster=predictions(wavelength_of_interest=self.WOI*self.res,profile=self.profile,raster=predict_image(self.VO.data,AI_model,buffer=AI_buffer,verbose=AI_verb,low_limit=AI_low_limit,high_limit=AI_high_limit,report_frequency=10**(1+2*int(not verbose))))
            else:
                prediction_raster=predictions(wavelength_of_interest=self.WOI*self.res,profile=self.profile,raster=predict_image(self.VO.smoothen(filter_size=presmooth_input).data,AI_model,buffer=AI_buffer,verbose=AI_verb,low_limit=AI_low_limit,high_limit=AI_high_limit,report_frequency=10**(1+2*int(not verbose))))
            if AI_raw_output is not None:
                    prediction_raster.write_single_band_tif(AI_raw_output,dtype=rasterio.float32)
    ...
    def dune_predictions_raw_cleaned(self,method='slopes',AI_treshold=0.9,AI_model=None,AI_raw_output=None,AI_buffer=5,AI_low_limit=0.0010634725,AI_high_limit=0.99893653,slope_treshold=0.1,LoG_treshold=0.05,size_treshold=50,hole_treshold=None,name=None,visual_control=False,visual_control_figsize=(15,10),verbose=True,output_to_file=None,AI_raw_precomputed=None,presmooth_input=0):
      
        if name is None:
            name=self.name
            
        if AI_raw_precomputed is None:
            prediction_raster = self.dune_predictions_raw(method=method,AI_treshold=AI_treshold,AI_model=AI_model,AI_raw_output=AI_raw_output,AI_buffer=AI_buffer,AI_low_limit=AI_low_limit,AI_high_limit=AI_high_limit,slope_treshold=slope_treshold,LoG_treshold=LoG_treshold,name=name,visual_control=visual_control,visual_control_figsize=visual_control_figsize,verbose=verbose,output_to_file=output_to_file,presmooth_input=presmooth_input)
        else:
            if verbose:
                print("Loading pre-computed raw AI output")
            prediction_raster=predictions(input_path=AI_raw_precomputed,from_path=True,wavelength_of_interest=self.WOI*self.res).tresholded(AI_treshold)
    ...
    def dune_predictions(self,method='slopes',AI_treshold=0.9,AI_model=None,AI_raw_output=None,AI_buffer=24,AI_low_limit=0.0010634725,AI_high_limit=0.99893653,slope_treshold=0.1,LoG_treshold=0.05,size_treshold=None,hole_treshold=None,name=None,visual_control=False,visual_control_figsize=(15,10),verbose=True,skeleton_visual_control=False,crop_x=[0.1,0.2],crop_y=[0.1,0.2],path_to_skeleton=None,save_crests=None,save_skeleton=None,output_to_file=None,output_precutting=None,AI_raw_precomputed=None,trough_levels=[0.5,0.2,0.1,0],crest_levels=[0.5,0.2,0.1,0.05],presmooth_detectors=0):
        ...
        if save_crests is not None:
            ...
            for_skeletonization=self.crest_detector(prediction_raster,presmooth=presmooth_detectors)
            skeleton=for_skeletonization.skeletonization(levels=crest_levels,boolean=True,visual_control=skeleton_visual_control,crop_x=crop_x,crop_y=crop_y)
            skeleton=skeleton.thin_skeleton()
            skeleton.nodata=0
    ...
    def trough_detector(self,mask,pad_mode='edge',VOVOP_treshold=0.03,low_LoG_limit=1.0411863,presmooth=0):
        if presmooth>0:
            VOVOP=self.VO.smoothen(filter_size=presmooth).multiply_by(-1).add(1).multiply_by(self.VOP.multiply_by(-1).add(1),name=r'$(1-$'"VO"r'$)\bullet(1-$'"VOP"r'$)$'" (dunes)").normalize_positive().add(-VOVOP_treshold).multiply_by(1/(1-VOVOP_treshold)) 
        else:
        ...
        if self.ignore_LoG:
            return mask.multiply_by(VOVOP,name="Trough detector").setup_for_skeletonization()
        else:
    ...
    def crest_detector(self,mask,pad_mode='edge',VOVON_treshold=0.05,presmooth=0):
        if presmooth>0:
            VOVON=self.VO.smoothen(filter_size=presmooth).multiply_by(self.VON.multiply_by(-1).add(1),name="VO"r'$\bullet(1-$'"VON"r'$)$'" (dunes)").normalize_positive().add(-VOVON_treshold).multiply_by(1/(1-VOVON_treshold))
        else:
        ...
        if self.ignore_LoG:
            return mask.multiply_by(VOVON,name="Crest detector").setup_for_skeletonization() 
        else:
...
class predictions(raster_data):
    ...
    def __init__(self,wavelength_of_interest=1500,**kwargs):
        super().__init__(**kwargs)
        self.WOI=wavelength_of_interest
    
    def cleaning_up_small_mess(self,size_treshold=50,hole_treshold=None,verbose=True,name=None,holes_too=True):
